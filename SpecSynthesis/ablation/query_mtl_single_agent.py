...
PROMPT_PATH = Path("prompts/prompt_for_single_agent.txt")
OUTPUT_BASE_DIR = Path("single_agent_mtls_ablation")
...
def run_ablation_study():
    ...
    print(f" Starting single agent MTL generation with {model}")
    ...
    combined_jsonl = output_dir / f"combined_{model}_single_agent.jsonl"
    total_saved = write_combined_jsonl_from_dir(output_dir, combined_jsonl)
    
    print(f"\n Single Agent MTL Generation Completed!")
    ...
    # Save metadata about the single agent run
    metadata = {
    ...
    metadata_file = output_dir / "single_agent_metadata.json"
