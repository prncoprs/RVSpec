...
def create_albedo_visualizations(alb_data, css_stats_list, dest_dir, ncss, nsteps, daylight_mask):
    ...
    if len(all_albedo) > 0:
        # 0-0.3
        all_albedo_clipped = all_albedo[all_albedo <= 0.3]
        ax2.hist(all_albedo_clipped, bins=50, range=(0, 0.3), edgecolor='black', alpha=0.7)
        ax2.set_xlabel('Albedo Intensity')
        ax2.set_ylabel('Frequency')
        ax2.set_title('Distribution of All Albedo Values')
        ax2.set_xlim([0, 0.3])  # X
        ax2.grid(True, alpha=0.3)
        
        mean_val = np.mean(all_albedo_clipped)
        median_val = np.median(all_albedo_clipped)
