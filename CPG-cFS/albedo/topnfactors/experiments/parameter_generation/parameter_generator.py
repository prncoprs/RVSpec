...
class CFSParameterGenerator:
    ...
    def generate_latin_hypercube_samples(self, n_samples: int, param_info: Dict[str, Dict[str, Any]]) -> List[Dict[str, float]]:
        ...
        for param_name, info in param_info.items():
            ...
            #  - 
            elif param_name == 'ORB_APOAPSIS_ALT':
                continue  # 
            else:
        ...
        for sample in scaled_samples:
            ...
            for i, param_name in enumerate(param_names):
                ...
                elif param_name == 'ORB_PERIAPSIS_ALT':
                    # 
                    param_set['ORB_PERIAPSIS_ALT'] = sample[i]
                    #  >= 
                    # 
                    if 'ORB_APOAPSIS_ALT' in param_info:
                        apo_max = param_info['ORB_APOAPSIS_ALT']['max']
                        #  >= 
                        param_set['ORB_APOAPSIS_ALT'] = np.random.uniform(sample[i], apo_max)
                    else:
                        # 
                        param_set['ORB_APOAPSIS_ALT'] = sample[i]
                else:
                    param_set[param_name] = sample[i]
                    
            # 
            if 'ORB_APOAPSIS_ALT' not in param_set and 'ORB_PERIAPSIS_ALT' in param_set:
                param_set['ORB_APOAPSIS_ALT'] = param_set['ORB_PERIAPSIS_ALT']         
    ...
    def substitute_orb_leo_parameters(self, param_set: Dict[str, float]) -> str:
        ...
        if 'ORB_PERIAPSIS_ALT' in param_set and 'ORB_APOAPSIS_ALT' in param_set:
            peri = param_set['ORB_PERIAPSIS_ALT']
            apo = param_set['ORB_APOAPSIS_ALT']
            
            #  >= 
            if apo < peri:
                print(f"Warning: Fixing invalid orbit (Apo {apo:.1f} < Peri {peri:.1f})")
                apo = peri  # 
            
            content = re.sub(
                r'(\s+)(\d+\.?\d*)\s+(\d+\.?\d*)\s+!\s+Periapsis & Apoapsis Altitude, km',
                f'\\g<1>{peri:.1f}      {apo:.1f}              !  Periapsis & Apoapsis Altitude, km',
                content
            )
    ...
    def validate_parameter_set(self, param_set: Dict[str, float]) -> bool:
        """"""
        # 
        if 'ORB_PERIAPSIS_ALT' in param_set and 'ORB_APOAPSIS_ALT' in param_set:
            if param_set['ORB_APOAPSIS_ALT'] < param_set['ORB_PERIAPSIS_ALT']:
                print(f"Invalid orbit: Apoapsis ({param_set['ORB_APOAPSIS_ALT']:.1f}) < "
                      f"Periapsis ({param_set['ORB_PERIAPSIS_ALT']:.1f})")
                return False
        
        # 
        if 'MASS' in param_set and param_set['MASS'] <= 0:
            print(f"Invalid mass: {param_set['MASS']}")
            return False
        
        # 
        if 'MOI_XX' in param_set and param_set['MOI_XX'] <= 0:
            print(f"Invalid MOI: {param_set['MOI_XX']}")
            return False
    ...
    def save_parameter_set(self, run_id: int, param_set: Dict[str, float], 
       ...
       # 
        if not self.validate_parameter_set(param_set):
            print(f"Warning: Parameter set {run_id} has invalid values, fixing...")
            # 
            if 'ORB_APOAPSIS_ALT' in param_set and 'ORB_PERIAPSIS_ALT' in param_set:
                if param_set['ORB_APOAPSIS_ALT'] < param_set['ORB_PERIAPSIS_ALT']:
                    param_set['ORB_APOAPSIS_ALT'] = param_set['ORB_PERIAPSIS_ALT']
                    # 
                    configs['orb_leo'] = self.substitute_orb_leo_parameters(param_set)
